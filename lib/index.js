  ...
  consume(entries) {
    ...
    for (const event of ordered) {
      ...
      if (event.type === "tool/call") {
        ...
        const name2 = this.#lastToolName ?? "\u5DE5\u5177";
        if (raw) {
          const flat = raw.replace(/\s+/g, " ").trim();
          const preview = flat.length > this.#resultPreviewChars ? `${flat.slice(0, this.#resultPreviewChars)}\u2026\uFF08\u5DF2\u622A\u65AD\uFF09` : flat;
          update = { type: "status", text: `${name2} \u5B8C\u6210\uFF1A${preview}` };
        } else {
          update = { type: "status", text: `\u6B63\u5728\u6574\u7406${name2}\u7684\u7ED3\u679C\u2026` };
        }
      }
    }
    return update;
...
var WeixinHarnessBridge = class {
      ...
      const answer = await this.#harness.ask(sessionId, text, {
        ...
        onUpdate: async (update) => {
          if (update?.type === "tool" && typeof update.name === "string") {
            await this.#forwardProgress(sender, `\u{1F527} \u6B63\u5728\u8C03\u7528\u5DE5\u5177\uFF1A${update.name}`, contextToken, runId);
  ...
  async #flushProgress(toUserId, contextToken, runId) {
    ...
    const send = () => this.#send(toUserId, text, contextToken, runId).catch((error) => this.#logger.warn?.("[dsh-weixin] failed to send a progress message:", error));
  ...
  async #start() {
    ...
    try {
      ...
      this.#bridge = new WeixinHarnessBridge({
        ...
        resultPreviewChars: this.#resultPreviewChars,
        progressThrottleMs: this.#progressThrottleMs
      });
...
async function createProductionController(ctx, config = {}, internals = {}) {
  ...
  const resultPreviewChars = Number.isInteger(config.progress?.resultPreviewChars) ? config.progress.resultPreviewChars : 150;
  const progressThrottleMs = Number.isInteger(config.progress?.throttleMs) ? config.progress.throttleMs : 1500;
