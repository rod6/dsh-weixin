  ...
  consume(entries) {
    ...
    for (const event of ordered) {
      ...
      if (event.type === "tool/call") {
        ...
        if (text) {
          const preview = text.length > this.#resultPreviewChars ? `${text.slice(0, this.#resultPreviewChars)}\u2026\uFF08\u5DF2\u622A\u65AD\uFF09` : text;
...
async function createProductionController(ctx, config = {}, internals = {}) {
  ...
  const resultPreviewChars = Number.isInteger(config.progress?.resultPreviewChars) ? config.progress.resultPreviewChars : 600;
